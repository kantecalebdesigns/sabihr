    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Tax configuration, pension, and statutory deduction management</p>
        </div>
    ...
    <>
      ...
      <div className="rounded-xl border border-[#efefef] bg-white p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div><span className="text-slate-500">Consolidated Relief:</span><br /><span className="font-medium">{formatNaira(config.consolidatedRelief)}</span></div>
          <div><span className="text-slate-500">GI Relief Rate:</span><br /><span className="font-medium">{config.grossIncomeRelief}%</span></div>
          <div><span className="text-slate-500">Minimum Tax Rate:</span><br /><span className="font-medium">{config.minimumTaxRate}%</span></div>
          <div><span className="text-slate-500">Effective From:</span><br /><span className="font-medium">{config.effectiveFrom}</span></div>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200/70">
              ...
              <th className="p-3 text-left text-xs font-medium text-slate-500">Employer %</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Voluntary %</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Computation Base</th>
          ...
          <tbody>
            {MOCK_PENSION_CONFIGS.map((p) => (
              <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                ...
                <td className="p-3 text-slate-500">{p.computationBase.join(" + ")}</td>
                <td className="p-3 text-slate-500">{p.effectiveFrom}</td>
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {MOCK_STATUTORY_BODIES.map((body) => {
          ...
          return (
            <div key={body.id} className="rounded-xl border border-[#efefef] bg-white p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-500">{body.code}</span>
                <span className={cn("text-xs font-medium", style?.color)}>
                  {style?.label}
                </span>
              </div>
              <p className="text-sm font-medium mb-1">{body.name}</p>
              <p className="text-xs text-slate-500">Rate: {body.rate}% of {body.computationBase}</p>
              <p className="text-xs text-slate-500">Next Due: {body.nextDueDate}</p>
            </div>
          );
        })}
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200/70">
              ...
              <th className="p-3 text-left text-xs font-medium text-slate-500">Employer %</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Employee %</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Frequency</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Next Due</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Status</th>
            </tr>
          </thead>
          <tbody>
            {MOCK_STATUTORY_BODIES.map((body) => {
              ...
              return (
                <tr key={body.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="p-3 text-slate-500 capitalize">{body.remittanceFrequency}</td>
                  <td className="p-3 text-slate-500">{body.nextDueDate}</td>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200/70">
              ...
              <th className="p-3 text-left text-xs font-medium text-slate-500">Amount Paid</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Balance</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Due Date</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Filed Date</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Status</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((item) => {
              ...
              return (
                <tr key={item.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="p-3 text-slate-500">{item.dueDate}</td>
                  <td className="p-3 text-slate-500">{item.filedDate || "—"}</td>
                  <td className={cn("p-3 text-sm font-medium", style?.color)}>{style?.label}</td>
                </tr>
              );
            })}
