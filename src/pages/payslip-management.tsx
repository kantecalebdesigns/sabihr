    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Design templates, generate, and distribute payslips</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => {}}><Download className="w-4 h-4 mr-2" />Export</Button>
      </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">{MOCK_PAYSLIP_TEMPLATES.length} templates configured</p>
        <Button size="sm" onClick={() => {}}><Plus className="w-4 h-4 mr-2" />New Template</Button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {MOCK_PAYSLIP_TEMPLATES.map((tpl) => {
          ...
          return (
            <div key={tpl.id} className="rounded-xl border border-[#efefef] bg-white p-5 hover:shadow-sm transition-shadow">
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 rounded-lg bg-[#f8fafc] flex items-center justify-center">
                  <Layout className="w-5 h-5 text-blue-600" />
                </div>
                <div className="flex items-center gap-2">
                  {tpl.isDefault && <span className="text-sm font-medium text-blue-700">Default</span>}
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-slate-500">
                <div>Layout: <span className="font-medium text-slate-900 capitalize">{tpl.layout}</span></div>
                <div>Watermark: <span className="font-medium text-slate-900">{tpl.watermark ? "Yes" : "No"}</span></div>
                <div>YTD Totals: <span className="font-medium text-slate-900">{tpl.showYtdTotals ? "Yes" : "No"}</span></div>
                <div>QR Code: <span className="font-medium text-slate-900">{tpl.showQrCode ? "Yes" : "No"}</span></div>
              </div>
              <div className="flex items-center gap-2 mt-4 pt-3 border-t border-[#efefef]">
                <Button variant="outline" size="sm" className="flex-1 h-8 text-xs" onClick={() => {}}><Eye className="w-3.5 h-3.5 mr-1" />Preview</Button>
                <Button variant="outline" size="sm" className="flex-1 h-8 text-xs" onClick={() => {}}><FileText className="w-3.5 h-3.5 mr-1" />Edit</Button>
              </div>
            </div>
          );
        })}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200/70">
              ...
              <th className="p-3 text-left text-xs font-medium text-slate-500">Generated</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Failed</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">File Size</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Generated By</th>
            ...
            {batches.map((batch) => {
              ...
              return (
                <tr key={batch.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="p-3 text-slate-500">{batch.fileSize || "—"}</td>
                  <td className="p-3 text-slate-500">{batch.generatedBy}</td>
                  ...
                  <td className="p-3">
                    ...
                    {batch.status === "completed" && <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => {}}><Download className="w-3.5 h-3.5 mr-1" />Download</Button>}
                  </td>
                </tr>
              );
            })}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200/70">
              ...
              <th className="p-3 text-left text-xs font-medium text-slate-500">Delivered</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Failed</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Scheduled</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Initiated By</th>
              <th className="p-3 text-left text-xs font-medium text-slate-500">Status</th>
            </tr>
          </thead>
          <tbody>
            {MOCK_DISTRIBUTIONS.map((dist) => {
              ...
              return (
                <tr key={dist.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="p-3 text-slate-500 text-xs">{new Date(dist.scheduledAt).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</td>
                  <td className="p-3 text-slate-500">{dist.initiatedBy}</td>
                  <td className={cn("p-3 text-sm font-medium", style?.color)}>{style?.label}</td>
                </tr>
              );
            })}
      ...
      <div className="rounded-xl border border-[#efefef] bg-white p-6">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-center justify-between py-3 border-b border-[#efefef] last:border-0">
              <div>
                <p className="text-sm font-medium">{item.label}</p>
                {item.detail && <p className="text-xs text-slate-500">{item.detail}</p>}
              </div>
              <span className={cn("text-sm font-medium", item.value.includes("Enabled") || item.value.includes("downloads") || item.value.includes("days") ? "text-emerald-700" : "text-gray-700")}>
                {item.value}
              </span>
            </div>
          ))}
        </div>
        <div className="mt-4"><Button variant="outline" size="sm" onClick={() => {}}>Edit Settings</Button></div>
      </div>
    </div>
  );
}
