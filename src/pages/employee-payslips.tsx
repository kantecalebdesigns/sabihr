  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">View your payslips, salary breakdown, and revision history</p>
      </div>
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-1"><Banknote className="w-4 h-4" /><span className="text-xs font-medium">Gross Pay</span></div>
          <p className="text-xl font-semibold">{formatNaira(payslip.grossPay)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-1"><DollarSign className="w-4 h-4" /><span className="text-xs font-medium">Net Pay</span></div>
          <p className="text-xl font-semibold text-emerald-600">{formatNaira(payslip.netPay)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-1"><TrendingUp className="w-4 h-4" /><span className="text-xs font-medium">Deductions</span></div>
          <p className="text-xl font-semibold text-red-600">{formatNaira(payslip.totalDeductions)}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-1"><Calendar className="w-4 h-4" /><span className="text-xs font-medium">Pay Date</span></div>
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Pay date: {payslip.payDate}</p>
          </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{MOCK_EMPLOYEE_PAYSLIPS.length} payslips available</p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <p className="text-xs text-muted-foreground">Gross: {formatNaira(p.grossPay)}</p>
                  </div>
                  <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border", style?.bg, style?.color)}>{style?.label}</span>
    ...
    <>
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div><span className="text-muted-foreground">Pay Grade:</span><br /><span className="font-medium">{b.payGrade}</span></div>
          <div><span className="text-muted-foreground">Structure:</span><br /><span className="font-medium">{b.salaryStructure}</span></div>
          <div><span className="text-muted-foreground">Effective:</span><br /><span className="font-medium">{b.effectiveDate}</span></div>
        </div>
      </div>
...
function RevisionHistory() {
  return (
    <>
      <p className="text-sm text-muted-foreground">{MOCK_SALARY_REVISIONS.length} salary revisions on record</p>
        ...
        {MOCK_SALARY_REVISIONS.map((rev) => (
          <div key={rev.id} className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              <div><span className="text-muted-foreground">Previous Gross</span><br /><span className="font-medium">{formatNaira(rev.previousGross)}</span></div>
              <div><span className="text-muted-foreground">New Gross</span><br /><span className="font-medium text-emerald-600">{formatNaira(rev.newGross)}</span></div>
              <div><span className="text-muted-foreground">Previous Net</span><br /><span className="font-medium">{formatNaira(rev.previousNet)}</span></div>
              <div><span className="text-muted-foreground">New Net</span><br /><span className="font-medium text-emerald-600">{formatNaira(rev.newNet)}</span></div>
            </div>
            <p className="text-xs text-muted-foreground mt-3">Approved by: {rev.approvedBy}</p>
          </div>
        ))}
      </div>
    </>
