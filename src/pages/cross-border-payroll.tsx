    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => alert("Cross-border payroll data exported successfully.")}>
            <Download className="w-4 h-4 mr-2" />
            Export
          </Button>
          <Button size="sm" onClick={() => alert("Add new currency configuration form would open here.")}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="text-left px-4 py-3 font-medium">Currency Name</th>
                <th className="text-left px-4 py-3 font-medium">Symbol</th>
                <th className="text-right px-4 py-3 font-medium">Exchange Rate (NGN)</th>
                <th className="text-center px-4 py-3 font-medium">Base</th>
                <th className="text-center px-4 py-3 font-medium">Auto-Update</th>
            ...
            <tbody>
              {filteredCurrencies.map((cur) => {
                ...
                return (
                  <tr key={cur.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{cur.lastUpdated}</td>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="text-right px-4 py-3 font-medium">Local Amount</th>
                <th className="text-right px-4 py-3 font-medium">Base Amount (NGN)</th>
                <th className="text-right px-4 py-3 font-medium">Rate</th>
                <th className="text-left px-4 py-3 font-medium">Bank / SWIFT</th>
                <th className="text-left px-4 py-3 font-medium">Method</th>
                <th className="text-left px-4 py-3 font-medium">Status</th>
                <th className="text-right px-4 py-3 font-medium">Fees (NGN)</th>
              </tr>
            </thead>
            <tbody>
              {filteredPayments.map((pmt) => {
                ...
                const methodLabel =
                  pmt.paymentMethod === "swift"
                    ...
                    : "Wallet";
                return (
                  <tr key={pmt.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3 font-medium">{pmt.employeeName}</td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[10px] text-muted-foreground font-mono">{pmt.swiftCode}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-gray-50 border-gray-200 text-gray-700">
                        {methodLabel}
                      </span>
                    </td>
                    <td className="px-4 py-3">
      ...
      {activeTab === "tax" && (
        <div className="grid gap-4">
          {filteredTaxConfigs.map((cfg) => (
            <div key={cfg.id} className="rounded-xl border border-border bg-card p-5">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{cfg.taxSystem} &middot; {cfg.currency} &middot; {cfg.employeeCount} employees</p>
                  </div>
                </div>
                <span
                  className={cn(
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 text-sm">
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Employee Tax</p>
                  <p className="font-medium">{cfg.employeeTaxRate}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Employer Tax</p>
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Health Insurance</p>
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Employee Burden</p>
                  <p className="font-semibold text-orange-600">{cfg.totalEmployeeBurden}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Employer Burden</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="text-left px-4 py-3 font-medium">Wallet Balances</th>
                <th className="text-right px-4 py-3 font-medium">Total (NGN)</th>
                <th className="text-left px-4 py-3 font-medium">Last Transaction</th>
                <th className="text-left px-4 py-3 font-medium">Status</th>
            ...
            <tbody>
              {filteredWallets.map((wallet) => {
                ...
                return (
                  <tr key={wallet.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-medium">{wallet.employeeName}</div>
                      <div className="text-[10px] text-muted-foreground">{wallet.employeeId}</div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1.5">
                        {wallet.wallets.map((wb) => (
                          <span
                            key={wb.currency}
                            className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-blue-50 border-blue-200 text-blue-700"
                          >
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{wallet.lastTransaction}</td>
                    <td className="px-4 py-3">
                      <span
                        className={cn(
                          "inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border",
